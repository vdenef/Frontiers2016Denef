...
## If want to limit to top taxa, then uncomment the following lines
merged_final_toptaxa <- prune_taxa(taxa_sums(merged_final) > 190, merged_final)
...
#combine into 1 dataframe, the divide cDNA by DNA data (adding 1 to each element to avoid zeroes)
...
########## cDNA:DNA clustering ##########
#One option is to identify genes that drive the PCoA spread, then plot over seasons, stations, fractions
#cDD_TAX=tax_table(merged_final_toptaxa)
#cDD_SAM=sample_data(subset_samples(subset_samples(merged_final,DNA == "cD"),names != "Su.EcD.110.DN"))
#cDD_OTU=otu_table(mf_cD2D_log,taxa_are_rows=TRUE)
#cDD_physeq = phyloseq(cDD_OTU,cDD_TAX,cDD_SAM)
#cDD_physeq = prune_taxa(taxa_sums(merged_final_toptaxa) > 500, cDD_physeq)
...
# in case you want to use the raw_merged data, uncomment next 7 lines of code
...
venn_frac_plot <- ggplot(venn_stats, aes(x = Test, y = Meanfrac, color = Test)) + geom_point(size = 5, alpha = 0.7) +
  facet_grid(~Fraction, scales="free", space="free_x") + ggtitle("Top 215 OTUs: Mean +/-SD") +
...
venn_stat_plot <- ggplot(venn_stats.1, aes(x = Test, y = Meanstat, color = Test)) + geom_point(size = 5, alpha = 0.7) +
  facet_grid(~Station, scales="free", space="free_x") + ggtitle("Top 215 OTUs: Mean +/-SD") +
...
venn_seas_plot <- ggplot(venn_stats.2, aes(x = Test, y = Meanseas, color = Test)) + geom_point(size = 5, alpha = 0.7) +
  facet_grid(~Season, scales="free", space="free_x") + ggtitle("Top 215 OTUs: Mean +/-SD") +
