...
## If want to limit to top taxa, then uncomment the following lines > 0.5% on average
merged_final_toptaxa <- prune_taxa(taxa_sums(merged_final) > 835, merged_final)
...
cDD_pca$rotation
# Betadiv pca
cDD_pca <- prcomp(t(mf_cD2D), scale=TRUE)
summary(cDD_pca)
cDD_pca2 <- cDD_pca$x
cDD_pca3 <- data.frame(cDD_pca2[, 1:4])
cDD_pca3$names <- row.names(cDD_pca3)
cDD_pca4 <- merge(cDD_pca3,data_final_df,by="names")
for(i in 1:length(cDD_pca4$Season)){
  cDD_pca4$Depth_Seas[i]<-paste(as.character(cDD_pca4$Depth_C[i]),
                                  as.character(cDD_pca4$Season[i]))}
cDD_pca_values <- cDD_pca$values
...
ScorePC1_Scaled<-predict(model)[,1]/lam
##Doesn’t work

Scaled_Scores<-t(t(scores)/lam)
Scaled_Scores
##Copy and paste


Scaled_Loadings<-t(t(model$rotation) * lam)
##Copy and paste
...
#Let's run SIMPER analysis to determine the OTU's that contribute most to the differences - we'll focus on 
for(i in 1:length(cDD_SAM$Season)){
  cDD_SAM$Frac_Seas[i]<-paste(as.character(cDD_SAM$Season[i]),
                                  as.character(cDD_SAM$Fraction[i]))}
sim <- with(cDD_SAM, simper(t(mf_cD2D), Frac_Seas))
summary(sim)
sim
View(cDD_TAX)

#pull out most different, highly active OTUs -- TO DO: REMOVE SAMPLES NOT MATCHED BETWEEN Sp and Su -- check names for other mismatches (DN vs DD for example), then calc diff ratios or ratio ratios and multiply times average cD signal (so needs to be added as well).
mf_cD2D_Sp<-data.frame(otu_table(subset_samples(cDD_physeq,Season == "Sp")))
mf_cD2D_Su<-data.frame(otu_table(subset_samples(cDD_physeq,Season == "Su")))
mf_cD2D_Su$OTU=rownames(mf_cD2D_Sp)
mf_cD2D_Sp$OTU=rownames(mf_cD2D_Su)
both = merge(mf_cD2D_Sp,mf_cD2D_Su)
mf_cD2D_Sp2Su = (both[, 2:48]+1) / (both[, 49:95]+1)
rownames(mf_cD2D) = rownames(mf_D)

#mf_cD2D_log.st = apply(mf_cD2D_log, 2, scale, center=TRUE, scale=TRUE)
#rownames(mf_cD2D_log.st) = rownames(mf_D)
#biplot.pcoa(cDD_bray_pcoa, Y=t(mf_cD2D_log.st), plot.axes = c(1,2), rn=NULL)
#help(biplot.pcoa)
...
cDD_physeq = phyloseq(cDD_OTU,cDD_TAX,cDD_SAM)
#cDD_physeq = prune_taxa(taxa_sums(merged_final_toptaxa) > 1000, cDD_physeq)
...
# Pick colors for group labels. n should match k above
otulabs.colors <- brewer.pal(n = 8, name = "Dark2")

# Make heatmap
ggplot(cDD.scale$data, aes(x = Group_cDvsD, y = OTU) ) +
  ...
  theme(
    ...
    axis.text.y = element_text(
      color =  otulabs.colors[cDD.scale$clusters]
    )
  )

########## cDNA:DNA ratio FL and PA Sp vs FL Su and Fa (without MLB?) DEseq or other stats ##########
...
cDD_PA_noMLB_noFall <- subset_samples(subset_samples(subset_samples(cDD_physeq, Station != "MLB"), Season != "Su"), Fraction != "B")
View(tax_table(cDD_PA_noMLB_noFall))
deseq_otu_cDD_PA_noMLB_noFall_raw <- deSEQ_noprune(cDD_PA_noMLB_noFall, ~ Season)
deseq_otu_cDD_PA_noMLB_noFall_raw$plotvalue <- log(as.numeric(deseq_otu_cDD_PA_noMLB_noFall_raw$baseMean),2)
deseq_otu_cDD_PA_noMLB_noFall_raw$sig <- as.factor((deseq_otu_cDD_PA_noMLB_noFall_raw$padj<0.05)*1)
deseq_otu_cDD_PA_noMLB_noFall_plot <- plot_deSEQ(deseq_otu_cDD_PA_noMLB_noFall_raw, "OTU cDNA:DNA PA samples")
...
deseq_otu_cDD_PA_noMLB_noFall_plot
#dev.off()
...
#dev.off()
View(deseq_otu_DNAcDNA_FL_raw)
View(deseq_otu_DNAcDNA_PA_raw)
#plot together
