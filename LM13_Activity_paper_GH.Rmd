...
#setwd("~/Documents/UMich/Data/Great_Lakes_Monitoring/JGI_sequencing/iTagplate1_cDNA_V4_tax_mapping")

# Source written functions in the files made by Marian and Michelle 
source("/Users/vdenef/Documents/UMich/Digital_Notebooks/LM13_Activity/r_functions.R")
source("/Users/vdenef/Documents/UMich/Digital_Notebooks/LM13_Activity/r_functions_github.R")
...
#physeq_mothur <- physeq_mothur_rdp
physeq_mothur <- physeq_top205

# We need to change the taxonomy names
tax_table(physeq_mothur) <- cbind(tax_table(physeq_mothur),row.names(tax_table(physeq_mothur)))
colnames(tax_table(physeq_mothur)) <- c("Kingdom","Phylum","Class","Order","Family","Genus","Species")

# removing non-bacterial reads
physeq_mothur <-subset_taxa(physeq_mothur, Kingdom == "Bacteria")

# checking read number distribution across samples
ggplot(data.frame(sum=sample_sums(physeq_mothur)),aes(sum)) + ylab("Number of Sequences per Sample") +
  geom_histogram(colour="black",fill="dodgerblue2", binwidth = 750)  + xlab("Total Sequences") + ggtitle("Raw Sample Read Counts")
mins<-min(sample_sums(physeq_mothur))
paste(c("The minimum sample read count is",mins))
means<-mean(sample_sums(physeq_mothur))
paste(c("The mean sample read count is",means))
maxs<-max(sample_sums(physeq_mothur))
...
noscale_merge <- merge_samples_mean(physeq_mothur, "Group", "floor")
...
pglom <- tax_glom(merged_final, taxrank = "Phylum")
  
# deseq without pruning low abundance
deseq_phy_DNAcDNA <- deSEQ_noprune(pglom, ~ DNA)
deseq_phy_DNAcDNA_plot <- plot_phylum_deSEQ(deseq_phy_DNAcDNA, "Phylum DNA:cDNA all samples")
deseq_phy_DNAcDNA_plot

# Agglomerate all otu's by given taxonomic level
pglom_order <- tax_glom(merged_final, taxrank = "Order")
  
# deseq without pruning low abundance
deseq_order_DNAcDNA <- deSEQ_noprune(pglom_order, ~ DNA)
deseq_order_DNAcDNA_plot <- plot_order_deSEQ(deseq_order_DNAcDNA, "Order DNA:cDNA all samples")
deseq_order_DNAcDNA_plot

# deseq without pruning low abundance - otu level
deseq_otu_DNAcDNA <- deSEQ_noprune(merged_final, ~ DNA)
deseq_otu_DNAcDNA$plotvalue <- log(as.numeric(deseq_otu_DNAcDNA$baseMean),2)
deseq_otu_DNAcDNA$sig <- as.factor((deseq_otu_DNAcDNA$padj<0.05)*1)
deseq_otu_DNAcDNA_plot <- plot_deSEQ(deseq_otu_DNAcDNA, "OTU cDNA:DNA all samples")
deseq_otu_DNAcDNA_plot

# deseq without pruning low abundance - otu level - per fraction - PA
merged_final_FL <- subset_samples(merged_final, Fraction != "E")
deseq_otu_DNAcDNA_FL <- deSEQ_noprune(merged_final_FL, ~ DNA)
deseq_otu_DNAcDNA_FL$plotvalue <- log(as.numeric(deseq_otu_DNAcDNA_FL$baseMean),2)
deseq_otu_DNAcDNA_FL$sig <- as.factor((deseq_otu_DNAcDNA_FL$padj<0.05)*1)
deseq_otu_DNAcDNA_plot_FL <- plot_deSEQ(deseq_otu_DNAcDNA_FL, "OTU cDNA:DNA FL samples")
pdf(file="/Users/vdenef/Documents/UMich/Papers/2015_Fujimoto_activity_Frontiers/Rplot_cDvsD_FL.pdf", width= 20, height=15, pointsize= 14, useDingbats=FALSE)
deseq_otu_DNAcDNA_plot_FL
dev.off()

# deseq without pruning low abundance - otu level - per fraction - FL
merged_final_PA <- subset_samples(merged_final, Fraction != "B")
deseq_otu_DNAcDNA_PA <- deSEQ_noprune(merged_final_PA, ~ DNA)
deseq_otu_DNAcDNA_PA$plotvalue <- log(as.numeric(deseq_otu_DNAcDNA_PA$baseMean),2)
deseq_otu_DNAcDNA_PA$sig <- as.factor((deseq_otu_DNAcDNA_PA$padj<0.05)*1)
deseq_otu_DNAcDNA_plot_PA <- plot_deSEQ(deseq_otu_DNAcDNA_PA, "OTU cDNA:DNA PA samples")
pdf(file="/Users/vdenef/Documents/UMich/Papers/2015_Fujimoto_activity_Frontiers/Rplot_cDvsD_PA.pdf", width= 20, height=15, pointsize= 14, useDingbats=FALSE)
deseq_otu_DNAcDNA_plot_PA
dev.off()
...
deseq_otu_DNAcDNA_PA_raw$sig <- as.factor((deseq_otu_DNAcDNA_PA_raw$padj<0.05)*1)
...
deseq_otu_DNAcDNA_FL_raw$sig <- as.factor((deseq_otu_DNAcDNA_FL_raw$padj<0.05)*1)
...
colnames(compare_df) <- c("DNA","cDNA","group","Season","Fraction","Station","Depth_C","otu_cDNA","otu_DNA","Shared","Uniq_cDNA","Uniq_DNA")
...
shared_df <- compare_df[,c("group","Shared","Season","Fraction","Station","Depth_C")]
shared_df$Test <- "Shared"
colnames(shared_df) = c("group","OTUs","Season","Fraction","Station","Depth_C","Test")
cDNAu_df <- compare_df[,c("group","Uniq_cDNA","Season","Fraction","Station","Depth_C")]
cDNAu_df$Test <- "Unique cDNA"
colnames(cDNAu_df) = c("group","OTUs","Season","Fraction","Station","Depth_C","Test")
DNAu_df <- compare_df[,c("group","Uniq_DNA","Season","Fraction","Station","Depth_C")]
DNAu_df$Test <- "Unique DNA"
colnames(DNAu_df) = c("group","OTUs","Season","Fraction","Station","Depth_C","Test")
...
##jpeg(filename="Abundance_top15.jpeg", width= 28, height=25, units= "cm", pointsize= 10, res=250)
otu_overlap_plot <- ggplot(venn_df, aes(y=OTUs , x=group, fill=Test)) + #coord_cartesian(xlim = c(0, 30)) + 
  guides(fill = guide_legend(reverse=TRUE)) + ggtitle("") + 
