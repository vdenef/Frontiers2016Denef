---
title: "16S itag analyses Denef et al., submitted to Frontiers in Microbiology"
...
###Make sure input files are where they need to be and set working directory 
# Adjust the paths below (setwd and source commands) to the working directory. Within the "LM13_Activity" directory there should be the following files: LM13_Activity_paper_GH.Rmd and LM13_Activity_function.R, as wel as a directory "Input_data" with the following files: , taxonomy table, OTU (shared) table, "metadata", "duplicate_metadata.txt", "AllLakes_depthprofile.csv and a folder named "Figures". 
setwd("/Users/vdenef/Documents/UMich/Digital_Notebooks/GitHub/LM13_Activity_GH/LM13_Activity/")

### Source written functions used below
...
### loading original mothur output and sample metadata
...
### loading rdp taxonomy (cutoff = 50)
...
### to generate this file from the files generated above, you have to replace cyanos and unclassifieds at phylum level in fwdb+silva with the rdp classification (cyanos are better annotated in RDP). We use a custom ruby script to do this. These commands are commented out below and are commands you would use in a shell window. The script and files match.file and match1.file are included in the Input_data folder
...
## commented out lines are used when using mothur or rdp taxonomy, not used for this manuscript's analysis
#tax_table(physeq) <- cbind(tax_table(physeq),row.names(tax_table(physeq)))
#colnames(tax_table(physeq)) <- c("Kingdom","Phylum","Class","Order","Family","Genus",Species")
## when using the fwdb taxonomy we need to add different headers after removing the last column, which contains no information except for 1 Verrucomicrobia taxon
...
### Combining technical replicate using merging function that calculates means of reps for each OTU, rounding down (floor)
...
###  Fixing the Metadata!
data_merged <- read.table("Input_data/LMSampleDescription_merged.tsv", sep="\t",header=T,row.names=1,as.is=T)
...
### Check sample read counts with merging samples before scaling 
...
### Now let's scale our read counts from our MERGED RAW data (using Michelle Berry's function based on McMurdie and Holmes, 2014)
...
data_final_df <- read.table("Input_data/LMSampleDescription_merged_final.tsv", sep="\t",header=T,row.names=1,as.is=T)
...
#same for raw_merged, but also add class name to Bacteroidetes phylum field
...
## beta-diversity DNA and cDNA count data: upgma + ADONIS + PCoA DNA + PCoA RNA + Procrustes (Fig. 2, Table 2)
```{r beta-diversity DNA and cDNA count data: upgma + ADONIS + PCoA DNA + PCoA RNA + Procrustes}

### upgma clustering as in Jones and Lennon, 2010 --- FOR FIGURE 2
...
### Let's run ADONIS using Michelle Berry's function --- FOR TABLE 2
...
### create PCoA plots for RNA and DNA seperately
...
PCoA_DNA_title <- paste("PCoA DNA data when considering",ntaxa(merged_final),"OTUs")
...
pcoa_BC34_DNA <- ggplot(bray_pcoa4_DNA, aes(Axis.3, Axis.4 * -1, color = Season, shape = Station, size = Fraction, fill = Depth_Seas)) +
  xlab(bray_axis3_DNA) + ylab(bray_axis4_DNA) + ggtitle(PCoA_DNA_title) +
  geom_point(alpha=0.9) + theme_bw() + #ylim(1, -1) + xlim(1, -1) +
  scale_color_manual(name = "season", breaks=c("Sp", "Su",  "Fa"),
                     labels = c("Spring", "Summer",  "Fall"), 
                     values = c("Fa" = "mediumblue", "Sp" = "darkgreen", "Su" = "red")) +
...
PCoA_RNA_title <- paste("PCoA RNA data when considering",ntaxa(merged_final),"OTUs")
...
pcoa_BC12_RNA <- ggplot(bray_pcoa4_RNA, aes(Axis.1, Axis.2 * -1, color = Season, shape = Station, size = Fraction, fill = Depth_Seas)) +
  xlab(bray_axis1_RNA) + ylab(bray_axis2_RNA) + ggtitle(PCoA_RNA_title) +
  geom_point(alpha=0.9) + theme_bw() + #ylim(1, -1) + xlim(1, -1) +
  scale_color_manual(name = "season", breaks=c("Sp", "Su",  "Fa"),
                     labels = c("Spring", "Summer",  "Fall"), 
                     values = c("Fa" = "mediumblue", "Sp" = "darkgreen", "Su" = "red")) +
...
pcoa_BC34_RNA <- ggplot(bray_pcoa4_RNA, aes(Axis.3, Axis.4 * -1, color = Season, shape = Station, size = Fraction, fill = Depth_Seas)) +
  xlab(bray_axis3_RNA) + ylab(bray_axis4_RNA) + ggtitle(PCoA_RNA_title) +
  geom_point(alpha=0.9) + theme_bw() + #ylim(1, -1) + xlim(1, -1) +
  scale_color_manual(name = "season", breaks=c("Sp", "Su",  "Fa"),
                     labels = c("Spring", "Summer",  "Fall"), 
                     values = c("Fa" = "mediumblue", "Sp" = "darkgreen", "Su" = "red")) +
...
### Procrustes analysis to determine whether DNA and RNA patterns are different or not
merged_final_RNA_procr <- subset_samples(subset_samples(merged_final,DNA == "cD"),names != "Su.EcD.110.DN")
norm_bray_RNA_procr <- vegdist(t(otu_table(merged_final_RNA_procr)), method = "bray")  # calculates the Bray-Curtis Distances
bray_pcoaveg_DNA <- cmdscale(norm_bray_DNA)
bray_pcoaveg_RNA <- cmdscale(norm_bray_RNA_procr)
proc_DNA_RNA <- procrustes(bray_pcoaveg_DNA, bray_pcoaveg_RNA, scale = FALSE, symmetric = FALSE)
summary(proc_DNA_RNA)
protest(bray_pcoaveg_DNA, bray_pcoaveg_RNA, permutations = how(nperm = 999))

### Partial Mantel test to look at how env factors have additional affect on RNA data after taking variation at the DNA level into account 
env_data <- data.frame(subset(sample_data(merged_final_DNA), select=c("Fraction", "Season", "DayNight","Lake", "Shore", "Depth_Q")))

# modifying factor values for calculation of Gower distance (note, numbers do not matter for binary measures, but are sclaed to distance between points for season (using month number) and location along transect (by using approximation of distance between locations))
env_data$Fraction[env_data$Fraction=="B"]<-"3"
env_data$Fraction[env_data$Fraction=="E"]<-"20"
env_data$Fraction<-as.numeric(env_data$Fraction)
env_data$Season[env_data$Season=="Sp"]<-"4"
env_data$Season[env_data$Season=="Su"]<-"7"
env_data$Season[env_data$Season=="Fa"]<-"9"
env_data$Season<-as.numeric(env_data$Season)
env_data$DayNight[env_data$DayNight=="D"]<-"1"
env_data$DayNight[env_data$DayNight=="N"]<-"2"
env_data$DayNight<-as.numeric(env_data$DayNight)
env_data$Lake[env_data$Lake=="MM"]<-"2"
env_data$Lake[env_data$Lake=="MLB"]<-"1"
env_data$Lake<-as.numeric(env_data$Lake)
env_data$Shore[env_data$Shore=="Off"]<-"20"
env_data$Shore[env_data$Shore=="Near"]<-"5"
env_data$Shore[env_data$Shore=="MLB"]<-"1"
env_data$Shore<-as.numeric(env_data$Shore)
env_data$Depth_Q<-as.numeric(env_data$Depth_Q)

# calculating gower distance basedo n environmental data and performing partial and standard Mantel test
env_gower <- daisy(env_data,metric = c("gower"))
mantel.partial(norm_bray_RNA_procr, env_gower, norm_bray_DNA, method = "pearson", permutations = 999, 
    strata = NULL, na.rm = FALSE, parallel = getOption("mc.cores"))
mantel(norm_bray_RNA_procr, env_gower)

# same analyses but with one env factor at a time: Fraction -- partial Mantel not significant
env_data_F <- data.frame(subset(sample_data(merged_final_DNA), select=c("Fraction")))
env_data_F$Fraction[env_data_F$Fraction=="B"]<-"3"
env_data_F$Fraction[env_data_F$Fraction=="E"]<-"20"
env_data_F$Fraction<-as.numeric(env_data_F$Fraction)
env_gower_F <- daisy(env_data_F,metric = c("gower"))
mantel.partial(norm_bray_RNA_procr, env_gower_F, norm_bray_DNA, method = "pearson", permutations = 999, 
    strata = NULL, na.rm = FALSE, parallel = getOption("mc.cores"))
mantel(norm_bray_RNA_procr, env_gower_F)

# one env factor at a time: Season -- partial Mantel significant
env_data_S <- data.frame(subset(sample_data(merged_final_DNA), select=c("Season")))
env_data_S$Season[env_data_S$Season=="Sp"]<-"4"
env_data_S$Season[env_data_S$Season=="Su"]<-"7"
env_data_S$Season[env_data_S$Season=="Fa"]<-"9"
env_data_S$Season<-as.numeric(env_data_S$Season)  
env_gower_S <- daisy(env_data_S,metric = c("gower"))
mantel.partial(norm_bray_RNA_procr, env_gower_S, norm_bray_DNA, method = "pearson", permutations = 999, 
    strata = NULL, na.rm = FALSE, parallel = getOption("mc.cores"))
mantel(norm_bray_RNA_procr, env_gower_S)

# one env factor at a time: DayNight -- partial Mantel not significant
env_data_DN <- data.frame(subset(sample_data(merged_final_DNA), select=c("DayNight")))
env_data_DN$DayNight[env_data_DN$DayNight=="D"]<-"1"
env_data_DN$DayNight[env_data_DN$DayNight=="N"]<-"2"
env_data_DN$DayNight<-as.numeric(env_data_DN$DayNight)
env_gower_DN <- daisy(env_data_DN,metric = c("gower"))
mantel.partial(norm_bray_RNA_procr, env_gower_DN, norm_bray_DNA, method = "pearson", permutations = 999, 
    strata = NULL, na.rm = FALSE, parallel = getOption("mc.cores"))
mantel(norm_bray_RNA_procr, env_gower_DN)

# one env factor at a time: Lake -- partial Mantel significant
env_data_L <- data.frame(subset(sample_data(merged_final_DNA), select=c("Lake")))
env_data_L$Lake[env_data_L$Lake=="MM"]<-"2"
env_data_L$Lake[env_data_L$Lake=="MLB"]<-"1"
env_data_L$Lake<-as.numeric(env_data_L$Lake)
env_gower_L <- daisy(env_data_L,metric = c("gower"))
mantel.partial(norm_bray_RNA_procr, env_gower_L, norm_bray_DNA, method = "pearson", permutations = 999, 
    strata = NULL, na.rm = FALSE, parallel = getOption("mc.cores"))
mantel(norm_bray_RNA_procr, env_gower_L)

# one env factor at a time: Shore -- partial Mantel not significant
env_data_Sh <- data.frame(subset(sample_data(merged_final_DNA), select=c("Shore")))
env_data_Sh$Shore[env_data_Sh$Shore=="Off"]<-"20"
env_data_Sh$Shore[env_data_Sh$Shore=="Near"]<-"5"
env_data_Sh$Shore[env_data_Sh$Shore=="MLB"]<-"1"
env_data_Sh$Shore<-as.numeric(env_data_Sh$Shore)
env_gower_Sh <- daisy(env_data_Sh,metric = c("gower"))
mantel.partial(norm_bray_RNA_procr, env_gower_Sh, norm_bray_DNA, method = "pearson", permutations = 999, 
    strata = NULL, na.rm = FALSE, parallel = getOption("mc.cores"))
mantel(norm_bray_RNA_procr, env_gower_Sh)

# one env factor at a time: Depth_Q -- partial Mantel not significant
env_data_D <- data.frame(subset(sample_data(merged_final_DNA), select=c("Depth_Q")))
env_data_D$Depth_Q<-as.numeric(env_data$Depth_Q)
env_gower_D <- daisy(env_data_D,metric = c("gower"))
mantel.partial(norm_bray_RNA_procr, env_gower_D, norm_bray_DNA, method = "pearson", permutations = 999, 
    strata = NULL, na.rm = FALSE, parallel = getOption("mc.cores"))
mantel(norm_bray_RNA_procr, env_gower_D)
```

## Top 200 taxa beta-diversity DNA and cDNA count data: upgma + ADONIS + PCoA DNA + PCoA RNA (Fig. 5A,B)
```{r top 200 taxa beta-diversity DNA and cDNA count data: upgma + ADONIS + PCoA DNA + PCoA RNA}

## limit phyloseq object to top 200 taxa
...
### determine how many sequences included for each sample when only considering 200 top OTUs
relab_toptaxa <- sample_sums(merged_final_toptaxa)/sample_sums(merged_final)*100
mean(relab_toptaxa)
sd(relab_toptaxa)

### ADD THE PROTEOBACTERIA CLASSES TO THE PHYLA
phy <- data.frame(tax_table(merged_final_toptaxa))
Phylum <- as.character(phy$Phylum)
Class <- as.character(phy$Class)
...
tax_table(merged_final_toptaxa) <- t

#same for raw_merged
phy <- data.frame(tax_table(raw_merged_toptaxa))
Phylum <- as.character(phy$Phylum)
Class <- as.character(phy$Class)
...
### ADD THE BACTEROIDETES CLASSES TO THE PHYLUM FIELD IN PHYLOSEQ OBJECT
for  (i in 1:length(Phylum)){ 
  if (Phylum[i] == "Bacteroidetes"){
    if (Class[i] == "unclassified"){
      Phylum[i] <- Phylum[i] 
...
tax_table(raw_merged_toptaxa) <- t

### upgma clustering as in Jones and Lennon, now just for the top 200 taxa
norm_bray_toptaxa <- vegdist(t(otu_table(merged_final_toptaxa)), method = "bray")  # calculates the Bray-Curtis Distances
upgma_toptaxa <- hclust(d = norm_bray_toptaxa,method = "average")
pdf(file="Figures/Rplot_upgma_bray_toptaxa.pdf", width= 20, height=15, pointsize= 14)
upgma_bray_toptaxa <- plot(upgma, hang = -1) ; upgma_bray_toptaxa
  dev.off()

#### Let's run ADONIS using Michelle Berry's function
phyloseq_to_adonis(merged_final_toptaxa, distmat = norm_bray, dist = "bray","Fraction")
phyloseq_to_adonis(merged_final_toptaxa, distmat = norm_bray, dist = "bray","Season")
phyloseq_to_adonis(merged_final_toptaxa, distmat = norm_bray, dist = "bray","Station")
phyloseq_to_adonis(merged_final_toptaxa, distmat = norm_bray, dist = "bray","DNA")
phyloseq_to_adonis(merged_final_toptaxa, distmat = norm_bray, dist = "bray","Depth_C")
phyloseq_to_adonis(merged_final_toptaxa, distmat = norm_bray, dist = "bray","DayNight")
phyloseq_to_adonis(merged_final_toptaxa, distmat = norm_bray, dist = "bray","Lake")
formula<-c("Fraction","Season","Lake","Station","Depth_C","DNA","DayNight")
phyloseq_to_adonis(merged_final_toptaxa, distmat = norm_bray, dist = "bray",formula)

### create PCoA plot for RNA and DNA seperately
# Betadiv pcoa Bray-Curtis DNA only ---- FOR FIGURE 5A
merged_final_toptaxa_DNA <- subset_samples(merged_final_toptaxa,DNA == "D")
norm_bray_toptaxa_DNA <- vegdist(t(otu_table(merged_final_toptaxa_DNA)), method = "bray")  # calculates the Bray-Curtis Distances
bray_pcoa_toptaxa_DNA <- pcoa(norm_bray_toptaxa_DNA)
bray_pcoa2_toptaxa_DNA <- bray_pcoa_toptaxa_DNA$vectors
bray_pcoa3_toptaxa_DNA <- data.frame(bray_pcoa2_toptaxa_DNA[, 1:4])
bray_pcoa3_toptaxa_DNA$names <- row.names(bray_pcoa3_toptaxa_DNA)
bray_pcoa4_toptaxa_DNA <- merge(bray_pcoa3_toptaxa_DNA,data_final_df,by="names")
for(i in 1:length(bray_pcoa4_toptaxa_DNA$Season)){
  bray_pcoa4_toptaxa_DNA$Depth_Seas[i]<-paste(as.character(bray_pcoa4_toptaxa_DNA$Depth_C[i]),
                                  as.character(bray_pcoa4_toptaxa_DNA$Season[i]))}
bray_values_toptaxa_DNA <- bray_pcoa_toptaxa_DNA$values
bray_rel_eigens_toptaxa_DNA <- bray_values_toptaxa_DNA$Relative_eig
bray_rel_eigen1_toptaxa_DNA <- bray_rel_eigens_toptaxa_DNA[1]
bray_rel_eigen1_percent_toptaxa_DNA <- round(bray_rel_eigen1_toptaxa_DNA * 100, digits = 1)
bray_rel_eigen2_toptaxa_DNA <- bray_rel_eigens_toptaxa_DNA[2]
bray_rel_eigen2_percent_toptaxa_DNA <- round(bray_rel_eigen2_toptaxa_DNA * 100, digits = 1)
bray_rel_eigen3_toptaxa_DNA <- bray_rel_eigens_toptaxa_DNA[3]
bray_rel_eigen3_percent_toptaxa_DNA <- round(bray_rel_eigen3_toptaxa_DNA * 100, digits = 1)
bray_rel_eigen4_toptaxa_DNA <- bray_rel_eigens_toptaxa_DNA[4]
bray_rel_eigen4_percent_toptaxa_DNA <- round(bray_rel_eigen4_toptaxa_DNA * 100, digits = 1)
bray_axis1_toptaxa_DNA <- paste("PCoA1:",bray_rel_eigen1_percent_toptaxa_DNA,"%")
bray_axis2_toptaxa_DNA <- paste("PCoA2:",bray_rel_eigen2_percent_toptaxa_DNA,"%")
bray_axis3_toptaxa_DNA <- paste("PCoA3:",bray_rel_eigen3_percent_toptaxa_DNA,"%")
bray_axis4_toptaxa_DNA <- paste("PCoA4:",bray_rel_eigen4_percent_toptaxa_DNA,"%")

PCoA_DNA_toptaxa_title <- paste("PCoA DNA data when considering",ntaxa(merged_final_toptaxa),"OTUs")

pdf(file="Figures/Rplot_PCOA12_BC_toptaxa_DNA.pdf", width= 10, height=7.5, pointsize= 14)
pcoa_BC12_toptaxa_DNA <- ggplot(bray_pcoa4_toptaxa_DNA, aes(Axis.1, Axis.2, color = Season, shape = Station, size = Fraction, fill = Depth_Seas)) +
  xlab(bray_axis1_toptaxa_DNA) + ylab(bray_axis2_toptaxa_DNA) + ggtitle(PCoA_DNA_toptaxa_title) +
  geom_point(alpha=0.9) + theme_bw() + #ylim(1, -1) + xlim(1, -1) +
  scale_color_manual(name = "season", breaks=c("Sp", "Su",  "Fa"),
                     labels = c("Spring", "Summer",  "Fall"), 
                     values = c("Fa" = "mediumblue", "Sp" = "darkgreen", "Su" = "red")) +
  ...
  theme(axis.text.x = element_text(colour="black", vjust=0.5, size=14), 
        ...
        legend.text = element_text(size = 12),
        ###LEGEND TOP RIGHT CORNER
        legend.position = "right"); pcoa_BC12_toptaxa_DNA
  dev.off()

pdf(file="Figures/Rplot_PCOA34_BC_toptaxa_DNA.pdf", width= 10, height=7.5, pointsize= 14)
pcoa_BC34_toptaxa_DNA <- ggplot(bray_pcoa4_toptaxa_DNA, aes(Axis.3, Axis.4 * -1, color = Season, shape = Station, size = Fraction, fill = Depth_Seas)) +
  xlab(bray_axis3_toptaxa_DNA) + ylab(bray_axis4_toptaxa_DNA) + ggtitle(PCoA_DNA_toptaxa_title) +
  geom_point(alpha=0.9) + theme_bw() + #ylim(1, -1) + xlim(1, -1) +
  scale_color_manual(name = "season", breaks=c("Sp", "Su",  "Fa"),
                     labels = c("Spring", "Summer",  "Fall"), 
                     values = c("Fa" = "mediumblue", "Sp" = "darkgreen", "Su" = "red")) +
  ...
  theme(axis.text.x = element_text(colour="black", vjust=0.5, size=14), 
        ...
        legend.text = element_text(size = 12),
        ###LEGEND TOP RIGHT CORNER
        legend.position = "right"); pcoa_BC34_toptaxa_DNA
  dev.off()


# Betadiv pcoa Bray-Curtis RNA only ---- FOR FIGURE 5B
merged_final_toptaxa_RNA <- subset_samples(merged_final_toptaxa,DNA == "cD")
norm_bray_toptaxa_RNA <- vegdist(t(otu_table(merged_final_toptaxa_RNA)), method = "bray")  # calculates the Bray-Curtis Distances
bray_pcoa_toptaxa_RNA <- pcoa(norm_bray_toptaxa_RNA)
bray_pcoa2_toptaxa_RNA <- bray_pcoa_toptaxa_RNA$vectors
bray_pcoa3_toptaxa_RNA <- data.frame(bray_pcoa2_toptaxa_RNA[, 1:4])
bray_pcoa3_toptaxa_RNA$names <- row.names(bray_pcoa3_toptaxa_RNA)
bray_pcoa4_toptaxa_RNA <- merge(bray_pcoa3_toptaxa_RNA,data_final_df,by="names")
for(i in 1:length(bray_pcoa4_toptaxa_RNA$Season)){
  bray_pcoa4_toptaxa_RNA$Depth_Seas[i]<-paste(as.character(bray_pcoa4_toptaxa_RNA$Depth_C[i]),
                                  as.character(bray_pcoa4_toptaxa_RNA$Season[i]))}
bray_values_toptaxa_RNA <- bray_pcoa_toptaxa_RNA$values
bray_rel_eigens_toptaxa_RNA <- bray_values_toptaxa_RNA$Relative_eig
bray_rel_eigen1_toptaxa_RNA <- bray_rel_eigens_toptaxa_RNA[1]
bray_rel_eigen1_percent_toptaxa_RNA <- round(bray_rel_eigen1_toptaxa_RNA * 100, digits = 1)
bray_rel_eigen2_toptaxa_RNA <- bray_rel_eigens_toptaxa_RNA[2]
bray_rel_eigen2_percent_toptaxa_RNA <- round(bray_rel_eigen2_toptaxa_RNA * 100, digits = 1)
bray_rel_eigen3_toptaxa_RNA <- bray_rel_eigens_toptaxa_RNA[3]
bray_rel_eigen3_percent_toptaxa_RNA <- round(bray_rel_eigen3_toptaxa_RNA * 100, digits = 1)
bray_rel_eigen4_toptaxa_RNA <- bray_rel_eigens_toptaxa_RNA[4]
bray_rel_eigen4_percent_toptaxa_RNA <- round(bray_rel_eigen4_toptaxa_RNA * 100, digits = 1)
bray_axis1_toptaxa_RNA <- paste("PCoA1:",bray_rel_eigen1_percent_toptaxa_RNA,"%")
bray_axis2_toptaxa_RNA <- paste("PCoA2:",bray_rel_eigen2_percent_toptaxa_RNA,"%")
bray_axis3_toptaxa_RNA <- paste("PCoA3:",bray_rel_eigen3_percent_toptaxa_RNA,"%")
bray_axis4_toptaxa_RNA <- paste("PCoA4:",bray_rel_eigen4_percent_toptaxa_RNA,"%")

PCoA_RNA_toptaxa_title <- paste("PCoA RNA data when considering",ntaxa(merged_final_toptaxa),"OTUs")

pdf(file="Figures/Rplot_PCOA12_BC_toptaxa_RNA.pdf", width= 10, height=7.5, pointsize= 14)
pcoa_BC12_toptaxa_RNA <- ggplot(bray_pcoa4_toptaxa_RNA, aes(Axis.1, Axis.2 * -1, color = Season, shape = Station, size = Fraction, fill = Depth_Seas)) +
  xlab(bray_axis1_toptaxa_RNA) + ylab(bray_axis2_toptaxa_RNA) + ggtitle(PCoA_RNA_toptaxa_title) +
  geom_point(alpha=0.9) + theme_bw() + #ylim(1, -1) + xlim(1, -1) +
  scale_color_manual(name = "season", breaks=c("Sp", "Su",  "Fa"),
                     labels = c("Spring", "Summer",  "Fall"), 
                     values = c("Fa" = "mediumblue", "Sp" = "darkgreen", "Su" = "red")) +
  ...
  theme(axis.text.x = element_text(colour="black", vjust=0.5, size=14), 
        ...
        legend.text = element_text(size = 12),
        ###LEGEND TOP RIGHT CORNER
        legend.position = "right"); pcoa_BC12_toptaxa_RNA
  dev.off()

pdf(file="Figures/Rplot_PCOA34_BC_toptaxa_RNA.pdf", width= 10, height=7.5, pointsize= 14)
pcoa_BC34_toptaxa_RNA <- ggplot(bray_pcoa4_toptaxa_RNA, aes(Axis.3, Axis.4 * -1, color = Season, shape = Station, size = Fraction, fill = Depth_Seas)) +
  xlab(bray_axis3_toptaxa_RNA) + ylab(bray_axis4_toptaxa_RNA) + ggtitle(PCoA_RNA_toptaxa_title) +
  geom_point(alpha=0.9) + theme_bw() + #ylim(1, -1) + xlim(1, -1) +
  scale_color_manual(name = "season", breaks=c("Sp", "Su",  "Fa"),
                     labels = c("Spring", "Summer",  "Fall"), 
                     values = c("Fa" = "mediumblue", "Sp" = "darkgreen", "Su" = "red")) +
  ...
  theme(axis.text.x = element_text(colour="black", vjust=0.5, size=14), 
        ...
        legend.text = element_text(size = 12),
        ###LEGEND TOP RIGHT CORNER
        legend.position = "right"); pcoa_BC34_toptaxa_RNA
  dev.off()

```

## RNA vs DNA overlap (Fig. 3A)
```{r RNA vs DNA overlap and replicates overlap}
### DNA vs RNA overlap 
...
## now make dataframe (compare_df) that adds samples to compare in column 1 and 2, metadata in next columns, and their corresponding otus in 8 and 9, then add numbers of overlapping, unique cDNA, unique DNA in column 10, 11, and 12, and relative abundances of these three categories in 13, 14, and 15
...
## now make dataframe rep_compare_df as above
#create new phyloseq object to manipulate
...
### now make boxplots showing fraction OTUs shared bewteen cDNA and DNA datasets relative to between replicates --- FOR FIGURE 3
...
# finally, the plot
pdf(file="Figures/Rplot_shared_DcD_reps_Season_boxplot.pdf", width= 5, height=5, pointsize= 14)
...
pdf(file="Figures/Rplot_shared_DcD_reps_Fraction_boxplot.pdf", width= 5, height=5, pointsize= 14)
...
# and here is the plot
pdf(file="Figures/Rplot_shared_DcD_reps_Station_boxplot.pdf", width= 5, height=5, pointsize= 14)
...
# and time to plot
pdf(file="Figures/Rplot_shared_DcD_reps_Depth_boxplot.pdf", width= 5, height=5, pointsize= 14)
...
## RNA vs DNA overlap (Fig. 3B)
```{r top 200 taxa RNA vs DNA overlap}
### DNA vs RNA overlap 
# first make a dataframe that has sample name and list of comma-delimited otus in it
temp <- otu_table(merged_final_toptaxa)
otu_list <- vector(mode="character", length=length(colnames(temp)))
for(i in 1:length(colnames(temp))){
  otu_list[i]<-as.character(colnames(temp)[i])}
otu_list_df <- data.frame(otu_list,otu_list,stringsAsFactors=FALSE)
...
## now make dataframe (compare_df) that adds samples to compare in column 1 and 2, metadata in next columns, and their corresponding otus in 8 and 9, then add numbers of overlapping, unique cDNA, unique DNA in column 10, 11, and 12, and relative abundances of these three categories in 13, 14, and 15
#create new phyloseq object to manipulate
temp_data <- sample_data(merged_final_toptaxa)
temp_otu <- otu_table(merged_final_toptaxa)
temp_taxa <- tax_table(merged_final_toptaxa)
temp_data$Sample = rownames(temp_data)
temp_phy <- merge_phyloseq(temp_otu, temp_taxa, temp_data)
...
# first make a dataframe that has sample name and list of comma-delimited otus in it
# we will do OTU overlap analysis only for OTUs witheld for cDNA-DNA overlap analysis
physeq_reps = prune_taxa(taxa_names(merged_final_toptaxa),physeq_toptaxa)
rep_temp <- otu_table(physeq_reps)
rep_otu_list <- vector(mode="character", length=length(colnames(rep_temp)))
for(i in 1:length(colnames(rep_temp))){
  rep_otu_list[i]<-as.character(colnames(rep_temp)[i])}
...
## make dataframe rep_compare_df as above 
#create new phyloseq object to manipulate
rep_temp_data <- sample_data(physeq_reps)
rep_temp_otu <- otu_table(physeq_reps)
rep_temp_taxa <- tax_table(physeq_reps)
...
### 1. how does overlap between cDNA:DNA change over the seasons relative to overlap between replicates 
overlap_title <- paste("Overlap cDNA and DNA and between replicates when considering ",ntaxa(merged_final_toptaxa)," OTUs")
...
pdf(file="Figures/Rplot_shared_DcD_reps_Season_boxplot_toptaxa.pdf", width= 5, height=5, pointsize= 14)
shared_DcD_reps_Season_boxplot_toptaxa <- ggplot(shared_DcD_reps_Season_try, aes(x = Season, y = Shared_rel, fill = Season)) + geom_point(size = 2) +
  geom_boxplot()  + ggtitle(overlap_title) +
  geom_text(data = shared_DcD_reps_Season_sigs, aes(label = siglabel, x = Season, y = .95), size = 4) +
  scale_fill_manual(name = "", limits=c("Sp", "Sp_rep", "Su", "Su_rep", "Fa", "Fa_rep"), 
                     values = c("Sp" = "grey","Su" = "grey","Fa" = "grey","Sp_rep" = "grey","Su_rep" = "grey","Fa_rep" = "grey"))+
  ...
  theme(plot.title = element_text(face="bold", size = 12),  #Set the plot title
                                 ...
                                 legend.text = element_text(size = 8),  #Set the legend text
                                 legend.position="none", #Default the legend position to the right
                                 plot.margin = unit(c(0.1, 0.1, 0.1, 0.1), "cm")); shared_DcD_reps_Season_boxplot_toptaxa#top, right, bottom, left
dev.off()
...
pdf(file="Figures/Rplot_shared_DcD_reps_Fraction_boxplot_toptaxa.pdf", width= 5, height=5, pointsize= 14)
shared_DcD_reps_Fraction_boxplot_toptaxa <- ggplot(shared_DcD_reps_Fraction_try, aes(x = Fraction, y = Shared_rel, fill = Fraction)) + geom_point(size = 2) +
  geom_boxplot()  + ggtitle(overlap_title) +
  geom_text(data = shared_DcD_reps_Fraction_sigs, aes(label = siglabel, x = Fraction, y = .95), size = 4) +
  scale_fill_manual(name = "", limits=c("B", "B_rep", "E", "E_rep"), 
                     values = c("B" = "grey","B_rep" = "grey","E" = "grey","E_rep" = "grey"))+
  ...
  theme(plot.title = element_text(face="bold", size = 12),  #Set the plot title
                                 ...
                                 legend.text = element_text(size = 8),  #Set the legend text
                                 legend.position="none", #Default the legend position to the right
                                 plot.margin = unit(c(0.1, 0.1, 0.1, 0.1), "cm")); shared_DcD_reps_Fraction_boxplot_toptaxa#top, right, bottom, left
dev.off()
...
pdf(file="Figures/Rplot_shared_DcD_reps_Station_boxplot_toptaxa.pdf", width= 5, height=5, pointsize= 14)
shared_DcD_reps_Station_boxplot_toptaxa <- ggplot(shared_DcD_reps_Station_try, aes(x = Station, y = Shared_rel, fill = Station)) + geom_point(size = 2) +
  geom_boxplot()  + ggtitle(overlap_title) +
  geom_text(data = shared_DcD_reps_Station_sigs, aes(label = siglabel, x = Station, y = .95), size = 4) +
  scale_fill_manual(name = "", limits=c("MLB", "MLB_rep", "MM15", "MM15_rep", "MM110", "MM110_rep"), 
                     values = c("MLB" = "grey","MLB_rep" = "grey","MM15" = "grey","MM15_rep" = "grey","MM110" = "grey","MM110_rep" = "grey"))+
  ...
  theme(plot.title = element_text(face="bold", size = 12),  #Set the plot title
                                 ...
                                 legend.text = element_text(size = 8),  #Set the legend text
                                 legend.position="none", #Default the legend position to the right
                                 plot.margin = unit(c(0.1, 0.1, 0.1, 0.1), "cm")); shared_DcD_reps_Station_boxplot_toptaxa#top, right, bottom, left
dev.off()
...
pdf(file="Figures/Rplot_shared_DcD_reps_Depth_boxplot_toptaxa.pdf", width= 5, height=5, pointsize= 14)
shared_DcD_reps_Depth_C_boxplot_toptaxa <- ggplot(shared_DcD_reps_Depth_C_try, aes(x = Depth_C, y = Shared_rel, fill = Depth_C)) + geom_point(size = 2) +
  geom_boxplot()  + ggtitle(overlap_title) +
  geom_text(data = shared_DcD_reps_Depth_C_sigs, aes(label = siglabel, x = Depth_C, y = .95), size = 4) +
  scale_fill_manual(name = "", limits=c("S", "S_rep", "M", "M_rep", "D", "D_rep"), 
                     values = c("S" = "grey","S_rep" = "grey","M" = "grey","M_rep" = "grey","D" = "grey","D_rep" = "grey"))+
  ...
  theme(plot.title = element_text(face="bold", size = 12),  #Set the plot title
                                 ...
                                 legend.text = element_text(size = 8),  #Set the legend text
                                 legend.position="none", #Default the legend position to the right
                                 plot.margin = unit(c(0.1, 0.1, 0.1, 0.1), "cm")); shared_DcD_reps_Depth_C_boxplot_toptaxa#top, right, bottom, left
dev.off()
```

## top 200 taxa RNA:DNA ratio data PCoA + Procrustes + ADONIS (Fig. 5C, Table 2)
```{r top 200 taxa RNA:DNA ratio data PCoA + Procrustes + ADONIS}

### cDNA:DNA PCoA ---- FOR FIG. 5C
#first separate otu_tables DNA and cDNA data, omit sample with no DNA match
mf_cD<-data.frame(otu_table(subset_samples(subset_samples(merged_final_toptaxa,DNA == "cD"),names != "Su.EcD.110.DN")))
mf_cD$OTU=rownames(mf_cD)
mf_D<-data.frame(otu_table(subset_samples(merged_final_toptaxa,DNA == "D")))
...
cDD_PCoA_title <- paste("PCoA cDNA:DNA ratios when considering",ntaxa(merged_final_toptaxa),"OTUs")
...
pdf(file="Figures/Rplot_cDD_PCOA12_EUC_toptaxa.pdf", width= 10, height=7.5, pointsize= 14)
cDD_pcoa_EUC12_toptaxa <- ggplot(cDD_euc_pcoa4, aes(Axis.1, Axis.2 * -1, color = Season, shape = Station, size = Fraction, fill = Depth_Seas)) +
  ...
  theme(axis.text.x = element_text(colour="black", vjust=0.5, size=14), 
        ...
        legend.text = element_text(size = 12),
        ###LEGEND TOP RIGHT CORNER
        legend.position = "right"); cDD_pcoa_EUC12_toptaxa
  dev.off()

pdf(file="Figures/Rplot_cDD_PCOA34_EUC_toptaxa.pdf", width= 10, height=7.5, pointsize= 14)
cDD_pcoa_EUC34_toptaxa <- ggplot(cDD_euc_pcoa4, aes(Axis.3, Axis.4 * -1, color = Season, shape = Station, size = Fraction, fill = Depth_Seas)) +
  ...
  theme(axis.text.x = element_text(colour="black", vjust=0.5, size=14), 
        ...
        legend.text = element_text(size = 12),
        ###LEGEND TOP RIGHT CORNER
        legend.position = "right"); cDD_pcoa_EUC34_toptaxa
  dev.off()

#Let's run ADONIS using Michelle's function --- FOR TABLE 2
...
### Procrustes analysis DNA - RNA - RNA:DNA ratios top 200 to determine correlation between ordination patterns
# first redo pcoa ordination in vegan, as procrustes package is written for vegan output
merged_final_toptaxa_RNA_procr <- subset_samples(subset_samples(merged_final_toptaxa,DNA == "cD"),names != "Su.EcD.110.DN")
norm_bray_toptaxa_RNA_procr <- vegdist(t(otu_table(merged_final_toptaxa_RNA_procr)), method = "bray")  # calculates the Bray-Curtis Distances
bray_pcoaveg_DNA_toptaxa <- cmdscale(norm_bray_toptaxa_DNA)
ordiplot(bray_pcoaveg_DNA_toptaxa, type="text")
bray_pcoaveg_RNA_toptaxa <- cmdscale(norm_bray_toptaxa_RNA_procr)
ordiplot(bray_pcoaveg_RNA_toptaxa, type="text")
eucl_pcoaveg_logcDD_toptaxa <- cmdscale(cDD_norm_euc)
ordiplot(eucl_pcoaveg_logcDD_toptaxa, type="text")

# run procrustes, plot the shifts in ordination space and test non-randomness between the two configurations
proc_DNA_RNA_toptaxa <- procrustes(bray_pcoaveg_DNA_toptaxa, bray_pcoaveg_RNA_toptaxa, scale = TRUE, symmetric = FALSE)
plot(proc_DNA_RNA_toptaxa)
protest(bray_pcoaveg_DNA_toptaxa, bray_pcoaveg_RNA_toptaxa, permutations = how(nperm = 999))
proc_DNA_logcDD_toptaxa <- procrustes(bray_pcoaveg_DNA_toptaxa, eucl_pcoaveg_logcDD_toptaxa, scale = TRUE, symmetric = FALSE)
plot(proc_DNA_logcDD_toptaxa)
protest(bray_pcoaveg_DNA_toptaxa, eucl_pcoaveg_logcDD_toptaxa, permutations = how(nperm = 999))
proc_RNA_logcDD_toptaxa <- procrustes(bray_pcoaveg_RNA_toptaxa, eucl_pcoaveg_logcDD_toptaxa, scale = TRUE, symmetric = FALSE)
plot(proc_RNA_logcDD_toptaxa)
protest(bray_pcoaveg_RNA_toptaxa, eucl_pcoaveg_logcDD_toptaxa, permutations = how(nperm = 999))
...
View(levels(t3$Species))
View(data.frame(t3$Species,t3$Phylum,t3$Class,t3$Order,t3$Family,t3$Genus,t3$Tribe))
```
